...
def test_repository_can_save_a_batch(session):
    ...
    temp_batch = repo.get("batch1")
    repo.delete(temp_batch)
    session.commit()
...
def delete_order_line(session, orderid):
    session.execute(
        "delete from order_lines where orderid=:orderid",
...
def test_repository_can_retrieve_a_batch_with_allocations(session):
    ...
    assert list(rows) == []
    delete_order_line(session, "order1")
